...
$Address2 = trim($profiles->Barangay) . ", Murcia";
...
<div class="doc" style="background-image: url('<?php echo base_url() ?>assets/img/OR2.png')">
    ...
    <div id="Agency" <?= $count <= 11 ? '' : 'hidden' ?> style="margin-top:1rem;">CITY TREASURER'S OFFICE</div>
    <div id="<?= $count <= 11 ? 'OR_num' : 'OR_num2' ?>"><?= $receipt['OR_number'] ?></div>
    <div style="word-wrap: break-word; white-space: normal; margin-top:1rem; margin-left:1rem;" id="<?= $count <= 11 ? 'Payor' : 'Payor2' ?>"><?= strtoupper($Payor) ?></div>
    <div style="margin-top:1rem; margin-left:1rem;" id="<?= $count <= 11 ? 'Address1' : 'Address12' ?>"><?= strtoupper($Address1) ?></div>
    <div style="margin-top:1rem; margin-left:1rem;" id="<?= $count <= 11 ? 'Address2' : 'Address22' ?>"><?= strtoupper($Address2) ?></div>
    <div id="<?= $count <= 11 ? 'Items' : 'Items2' ?>">
        <!-- <div id="Items1"> -->
        <!------------------------------------------------- 01-18-2020 ------------------------------------------------->
        <!-- <?php foreach ($items as $k => $item) { ?>
            <div style="display:flex; align-items:top; margin-right:1rem; font-size:120%;">
        ...
        <div class="testrow">
            <?php foreach ($items as $k => $item) {
                ...
                <div style="display:flex; align-items:top; margin-right:1rem; font-size:120%;">
                    <div style="width: 225px; font-weight:600;"><?= strtoupper($item['Fee']) ?></div>
                    <div style="margin-left: auto; font-weight:600;"><?= number_format($item['Amount'], 2) ?></div>
                </div>
            <?php
                if (@$k + 1 == $cnt) {
                    echo '</div>';
    ...
    <div id="Total" style=" margin-top:0rem; font-weight: 600;"><?= number_format($receipt['Amount_paid'], 2) ?></div>
    <div id="Words" style="margin-top:0rem;">&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
