...
class General_collection_service extends CI_Controller
{
    ...
    public function save_all_data()
    {
        try {
            if ($this->input->post('Payor', true) != null && !empty($this->input->post('Accountable_form_number', true)) && !empty($this->input->post('Particulars', true))) {
                ...
                $this->MCollection->Other_remarks     =   $this->input->post('Other_remarks', true);
    ...
    public function save_data_with_bank()
    {
        try {
            if ($this->input->post('Payor', true) != null && !empty($this->input->post('Particulars', true))) {
                ...
                $this->MCollection->Total_amount = $this->input->post('C_amount', true);
    ...
    public function save_data_mixed_pmnt()
    {
        try {
            if (!empty($this->input->post('Payer')) || !empty($this->input->post('Particulars'))) {
                ...
                $this->MCollection->Total_amount     =   $this->input->post('Mix_cheque_amount', true);
    ...
    // SAVE NEW GROUP PARTICULAR ANGELO 7/17/23
    public function save_new_group_particulars(){
        try {
            $this->MCollection->Group          = $this->input->post('Group', true);
            $this->MCollection->NewGroup  = $this->input->post('N_group', true);
            $this->MCollection->Particulars  = $this->input->post('Particulars', true);
            $this->MCollection->Amounts  = $this->input->post('Amounts', true);
           
            // if (empty($this->input->post('Particulars', true))) {
            //     throw new Exception("Please select collection type", true);
            // }
            $this->MCollection->save_group_particular();
        } catch (Exception $msg) {
            echo json_encode(array('error_message' => $msg->getMessage(), 'has_error' => true));
        }
    ...
    public function search_particular_group()
    {
        try {
            if (!empty($this->input->post('particular', true))) {
                $this->MCollection->Particular = $this->input->post('particular', true);
                $this->MCollection->getParticularGroup();
            }
        } catch (Exception $msg) {
            echo json_encode(array('error_message' => $msg->getMessage(), 'has_error' => true));
        }
    ...
    public function delete_particulars(){
        try {
            $this->MCollection->ParticularID = $this->input->post('particular_ID', true);
            $this->MCollection->delete_particulars();
            echo json_encode(array('has_error' => false));
        } catch (Exception $msg) {
            echo json_encode(array('error_message' => $msg->getMessage(), 'has_error' => true));
        }
    ...
    public function update_particulars(){
        try {
            $this->MCollection->particular_ID = $this->input->post('particular_ID', true);
            $this->MCollection->particular_amount = $this->input->post('particular_amount', true);
            $this->MCollection->update_particular_amount();
            echo json_encode(array('has_error' => false));
        } catch (Exception $msg) {
            echo json_encode(array('error_message' => $msg->getMessage(), 'has_error' => true));
        }
