    ...
    <section class="content" style="font-weight: bold;">
        ...
        <body onload="window.print()">
            <div class="doc" style="background-image: url('<?php echo base_url() ?>assets/img/OR2.png')"> 
                ...
                <div id="Agency">CITY TREASURER'S OFFICE <label style="margin-left: 45px;"> GEN. </label></div>
                <div id="OR_num" style="margin-left: 20px;"><?= @$Data->or_number?></div>
                <div id="Payor" style="margin-left: -10px; word-wrap: break-word; white-space: normal;"><?= (empty(@$Data->payor) ? strtoupper(@$Data->paid_by) : strtoupper(@$Data->payor)) ?></div> <!--MODIFIED BY KYLE 10-27-2023 -->
                <div id="Address1" style="margin-left: -10px; margin-top: 20px;"><?= strtoupper(@$Data->address) ?></div> <!--MODIFIED BY KYLE 10-27-2023 -->
                <div id="Items" style="margin-left: 0px;">
                    <?php foreach (@$Data->particulars as $item) { ?>
                        ...
                        <div style="display:flex; align-items:top;">
                            <!-- <div style="width: 300px; height: 20px;"><?= strtoupper(@$item->particular) ?> <?= empty(@$item->remarks ? "" : '('.strtoupper(@$item->remarks).')') ?></div> -->
                            <div style="width: 300px;"><?= strtoupper(@$item->particular) ?> <?php if(@$item->remarks == NULL){ echo "";}else{ echo '('.strtoupper(@$item->remarks).')';}?></div>
